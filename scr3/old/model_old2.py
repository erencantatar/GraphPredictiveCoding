...
#             axs[idx].imshow(generated_imgs[idx].cpu().detach().numpy(), cmap='gray')
#             axs[idx].axis("off")
#             axs[idx].set_title(f"{labels[idx].item()}")

#         plt.tight_layout()

#         if wandb_logging:
#             wandb.log({
#                 f"{folder}/Images": wandb.Image(fig, caption=f"Generated Images at Epoch {self.epoch}")
#             })
...
#             mosaic_rows = [
#                 [str(i) for i in range(num_snapshots)],
#                 ["F"] * (num_snapshots // 2) + ["G"] * (num_snapshots - num_snapshots // 2),
#             ]
...
#             # ====== Plot Energy with Dual Y-Axis ======
#             if sum(self.lst_internal_error) > 0 and sum(self.lst_sensor_error) > 0:
#                 fig_energy, ax1 = plt.subplots(figsize=(8, 4))

#                 color1 = 'tab:red'
#                 ax1.set_xlabel("Time Step")
#                 ax1.set_ylabel("Internal Energy", color=color1)
#                 ax1.plot(self.lst_internal_error, color=color1, label="Internal Energy")
#                 ax1.tick_params(axis='y', labelcolor=color1)

#                 ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

#                 color2 = 'tab:blue'
#                 ax2.set_ylabel("Sensory Energy", color=color2)
#                 ax2.plot(self.lst_sensor_error, color=color2, label="Sensory Energy")
...
#                 if wandb_logging:
#                     wandb.log({
#                         f"{folder}/Energy_Two_Y_Axis": wandb.Image(fig_energy, caption=f"Dual Y-Axis Energy Plot at Epoch {self.epoch}")
#                     })
#                     print(f"Logged dual Y-axis energy plot to WandB under '{self.task}/Energy_Two_Y_Axis' at epoch {self.epoch}")

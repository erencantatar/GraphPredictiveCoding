...
# --- Configuration ---
use_all_runs = False
run_ids = ["ijdt4bad", "5mqvzvkb"]  # Only used if use_all_runs=False
...
if len(flat_weights_all) == 0:
    print("✗ No valid weight files found. Check run_ids or file paths.")
    exit()
...
for run_id, flat in flat_weights_by_run:
    centered = flat - flat_weights_all.mean(axis=0)
...
for run_id, data in projected_pca_data:
    ...
    plt.quiver(x, y, u, v, angles='xy', scale_units='xy',
               ...
               color=color, alpha=alpha_value, label=run_id)
plt.xlabel("PCA 1")
plt.ylabel("PCA 2")

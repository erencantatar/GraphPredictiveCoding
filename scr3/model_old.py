...
#         # plot trace
#         if self.trace_data:  # Make sure there's trace data available!
#             trace_images = np.stack(self.trace_data)  # shape: [T_test, batch_size, 28, 28]

#             # Select one image from the batch (e.g., first image)
#             single_trace = trace_images[:, 0]  # shape: [T_test, 28, 28]

#             # Plot the time steps for this single image
#             fig_trace, axs_trace = plt.subplots(1, self.T_test, figsize=(self.T_test * 2, 2))
#             for t in range(self.T_test):
#                 axs_trace[t].imshow(single_trace[t], cmap='gray')
#                 axs_trace[t].axis("off")
#                 axs_trace[t].set_title(f"T={t}")
...
#                 save_dir = f"trained_models/{self.task}/"
#                 os.makedirs(save_dir, exist_ok=True)
#                 filename_trace = f"{save_dir}/trace_single_img_epoch_{self.epoch}.png"
#                 plt.savefig(filename_trace)
#                 print(f"Saved single image trace to: {filename_trace}")

#             # Log to wandb
#             if wandb_logging:
#                 wandb.log({
#                     f"Generation/Single_Trace_Epoch_{self.epoch}": wandb.Image(
#                         fig_trace, caption=f"Single Trace Evolution at Epoch {self.epoch}"
#                     )
#                 })
#                 print(f"Logged single trace to WandB at epoch {self.epoch}")

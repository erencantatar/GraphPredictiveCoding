...
from models.MessagePassing import PredictionMessagePassing, DeltaXUpdate # ,ValueMessagePassing
...
class PCGNN(torch.nn.Module): 
    def __init__(self, num_vertices, sensory_indices, internal_indices, 
        ...
        super(PCGNN, self).__init__()  # 'add' aggregation
        ...
        self.lr_values , self.lr_weights = lr_params  
        ...
        # self.values_mp      = DeltaXUpdate(dfdx=self.f_prime)
        self.delta_x_MP      = DeltaXUpdate(dfdx=self.f_prime)
        ...
        num_features = 1 # Number of features per node, 3 for RGB
        self.values_dummy = torch.nn.Parameter(torch.zeros(self.batchsize * self.num_vertices, num_features,  device=self.device), requires_grad=True) # requires_grad=False)                
        self.values = None
        self.errors = None
    ...
    def get_sparse_weight(self, w_ij=True):
        ...
        if w_ij:
          sparse_weights = self.weights[single_graph_edge_index[0], single_graph_edge_index[1]]
        else:
          w_c = self.w.clone()
    ...
    def init_modes(self, batch_example):
        ...
        self.batch_size = batch_example.num_graphs
        ...
        print("Internal indices batch:", self.internal_indices_batch.shape, len(self.internal_indices) , self.batch_size)
    ...
    def update_values(self):
        ...
        if self.update_rules == "Salvatori":
            ...
            with torch.no_grad():
                ...
                num_features = 1
                edge_weights_batched = self.weights.repeat(self.batch_size).view(-1, num_features)
                ...
                # print("edge_weights_batched[mask]", edge_weights_batched[mask].shape)
                print("self.data.edge_index", self.data.edge_index.shape)
                # print("subgraph_edge_index", subgraph_edge_index.shape)
                ...
                #     x=self.values,
                #     errors=self.errors,  # Already precomputed
                #     edge_index= self.edge_index[:, self.edges_2_update] ,
                #     # edge_weight=edge_weights_batched
                #     edge_weight=edge_weights_batched[self.edges_2_update]
                # )
    ...
    def get_predictions(self, data):
        ...
        if self.update_rules == "Salvatori":
            ...
            self.prediction = self.prediction_mp(self.values, self.edge_index, edge_weights_batched)
    ...
    def energy(self):
        ...
        if self.update_rules == "Salvatori":
            ...
            print("self.errors shape", self.errors.shape)
            print("self.internal_indices]", len(self.internal_indices))
            print("self.sensory_indices", len(self.sensory_indices))
            print("len nodes2update", len(self.nodes_2_update))

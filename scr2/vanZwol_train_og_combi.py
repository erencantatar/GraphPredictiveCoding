...
class PredictiveCodingLayer(MessagePassing):
    def __init__(self, f, dfdx):
        super().__init__(aggr='add', flow="source_to_target")  # Keep source_to_target for both
        self.f = f
        self.dfdx = dfdx  # Activation derivative

    def forward(self, x, edge_index, weight, error=None):
        """
        Compute prediction μ and update Δx.
        """

        # Step 1: Compute predictions μ (same as PredictionMessagePassing)
        mu = self.propagate(edge_index, x=x, weight=weight, mode="mu")

        # Step 2: Compute prediction error ε = x - μ
        epsilon = x - mu

        # Step 3: Compute Δx using W^T (same as GradientMessagePassing)
        out = self.propagate(edge_index, x=x, error=epsilon, weight=weight, mode="delta_x")

        # Step 4: Compute final Δx update
        delta_x = epsilon - out

        return epsilon, mu, delta_x

    def message(self, x_i, x_j, weight, mode, error_j=None):
        """
        Unified message function that switches behavior based on mode.
        """
        if mode == "mu":
            return self.f(x_j) * weight  # Forward pass: Compute μ
        elif mode == "delta_x":
            return self.dfdx(x_i) * error_j * weight  # Backward pass: Compute Δx

    def update(self, aggr_out):
        """
        Return accumulated messages without scatter.
        """
        return aggr_out  # Direct pass-through
...
class PCgraph(PCmodel):
    ...
    def __init__(self, lr_x: float, T_train: int, T_test: int, structure: PCGStructure, 
        ...
        ).to(DEVICE)  # Concatenate and offset indices
        ...
        # dfdx = lambda x: 1 - torch.tanh(x)**2

        # Instantiate PredictiveCodingLayer
        self.main_MP = PredictiveCodingLayer(f=self.structure.f, dfdx=self.structure.dfdx)
    ...
    def reset_nodes(self, batch_size=1):
        # print("reset_nodes with batch_size: ", batch_size)
    ...
    def update_w(self):
        ...
        self.e = self.e.view(self.batch_size, self.structure.N)
        self.x = self.x.view(self.batch_size, self.structure.N)

        out = -torch.matmul(self.e.T, self.structure.f(self.x))
        if self.mask is not None:
            out *= self.mask
        self.dw = out
    ...
    def update_xs(self, train=True):
        ...
        #     print(self.structure.N)
        #     print(-self.structure.shape[2])
        ...
        for t in range(T): 
            ...
            weight_vector = self.w[self.edge_index[0], self.edge_index[1]]

            batched_weights = weight_vector.repeat(self.batch_size).to(DEVICE).unsqueeze(-1)
            ...
            epsilon, mu, delta_x = self.main_MP.forward(
                self.x.view(-1, 1).to(DEVICE),
                self.batched_edge_index,
                batched_weights,
            )

            # Ensure correct shape handling
            # delta_x = delta_x.view(self.batch_size, N)[:, lower:upper]
            # print("delta_x", delta_x.shape)
            # print(self.batch_size, self.structure.N)
            # delta_x = delta_x.view(self.batch_size, self.structure.N) 
            delta_x = delta_x.view(self.batch_size, self.structure.N) 
            self.epsilon = epsilon.view(self.batch_size, self.structure.N) 

            # print("SUM (epsilon)", torch.sum(self.e - self.epsilon))
            # assert torch.allclose(self.e, self.epsilon, atol=1e-5)
            dEdx = delta_x[:, di:upper]

            dEdx_true = self.structure.grad_x(self.x.to(DEVICE), self.e.to(DEVICE), self.w.to(DEVICE), self.b.to(DEVICE), train=train) # only hidden nodes
            
            # print(torch.sum(dEdx - dEdx_true))
            # assert torch.allclose(dEdx, dEdx_true, atol=1e-5)
            ...
            clipped_dEdx = torch.clamp(dEdx_true, -1, 1)
...
# single_graph = graph.edge_index.flip(0)
